...
int remove_DDRC (EFI_ACPI_TABLE_PROTOCOL *acpi_table) {
    uintn_t table_key;

    char ssdt[] = {
        0x53, 0x53, 0x44, 0x54, 0x65, 0x00, 0x00, 0x00, 0x02, 0x2d, 0x44, 0x49,
        0x58, 0x59, 0x45, 0x53, 0x44, 0x49, 0x58, 0x59, 0x45, 0x53, 0x30, 0x31,
        0x00, 0x00, 0x00, 0x00, 0x49, 0x4e, 0x54, 0x4c, 0x20, 0x10, 0x22, 0x20,
    ...
    efi_status_t ret = acpi_table->InstallAcpiTable(acpi_table, &ssdt, sizeof(ssdt), &table_key);
    if (EFI_SUCCESS != ret) {
        printf("failed InstallAcpiTable SSDT remove DDR2 and DDR3\n");
        return 1;
    }
    printf("SSDT remove DDR2 and DDR3 registered %d\n", table_key);
    return 0;
}
